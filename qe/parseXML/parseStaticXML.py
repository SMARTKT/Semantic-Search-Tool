...
if __name__ == "__main__":
    ...
    parser.add_argument(dest='OUTPUTFOLDER',action='store', help='Path to the folder which would store intermediate output files')
    ...
    with open(os.path.join(OUTPUTFolder, 'name_tokens.csv'), 'w') as myfile:
        wr = csv.writer(myfile, delimiter = '\n')
        wr.writerow(list(all_name_tokens))

    pickle.dump( all_name_tokens, open( os.path.join(OUTPUTFolder, "name_tokens.p"), "wb" ) )
    pickle.dump( all_files, open( os.path.join(OUTPUTFolder, "files.p"), "wb" ) )
